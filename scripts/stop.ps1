...
function Stop-Tree([int]$Pid, [string]$Label) {
    try {
        taskkill /PID $Pid /T /F 2>$null | Out-Null
        if ($LASTEXITCODE -eq 0) {
            Write-Host "  [OK] stopped $Label (pid $Pid)" -ForegroundColor Green
            $script:stopped++
        }
    } catch { }
}
...
# ─── Fallback: stray processes referencing this repository ─────────────
...
foreach ($p in $stray) {
    if ($p.CommandLine -match "next|uvicorn|app\.main:app") {
